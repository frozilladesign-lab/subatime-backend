...
/** Pure sidereal chart + transit calculation engine. No NestJS, no DB. */
export class JyotishaChartEngine {
  /** Release native Swiss Ephemeris handles. Call once on process shutdown. */
  close(): void {
    try {
      close();
    } catch {
  ...
  generate(dto: GenerateChartInput): GeneratedChart {
    const coordinates = this.resolveCoordinates(dto.birthPlace, dto.latitude, dto.longitude);
    this.validateCoordinates(coordinates.lat, coordinates.lon);
    ...
    if (process.env.CHART_ENGINE === 'legacy') {
      return this.stampDegraded(this.generateLegacy(dto, birthMoment.utc, coordinates, birthMoment));
    }

    if (process.env.CHART_ENGINE === 'strict-swiss') {
      try {
        return this.generateSwiss(dto, birthMoment.utc, coordinates, birthMoment);
      } catch (err) {
        // Strict mode: no silent fallback to legacy math. The caller must handle this error
        // (e.g. surface a retry or "chart temporarily unavailable" state) rather than receive
        // a chart with an unverified ascendant.
        throw new ChartCalculationError(
          `Swiss Ephemeris chart calculation failed in strict-swiss mode: ${String(err)}`,
          'SWISS_EPHEMERIS_FAILED',
          err,
        );
      }
    }
    ...
    try {
      ...
      // KNOWN ACCURACY GAP: the legacy ascendant formula has an unresolved ~270° error
      // (verified against Swiss Ephemeris output, not yet root-caused), and the legacy
      // Mars/Mercury/Venus mean-longitude approximations drift 40-54° from Swiss Ephemeris
      // (missing orbital eccentricity correction). Sun/Moon/Jupiter/Saturn/Rahu/Ketu are
      // accurate to ~1°. This means a chart from this fallback can have a WRONG lagna and
      // wrong Mars/Mercury/Venus signs/houses — which corrupts dasha lord, dosha (Manglik)
      // detection, and scoring. Log loudly so this is never a silent failure mode.
      // This non-strict path is for development/testing convenience only — production
      // deployments should run with CHART_ENGINE=strict-swiss so this gap can never occur silently.
      console.error(
        `[jyotisha-engine] Swiss Ephemeris chart failed — falling back to legacy engine, ` +
          `which has a KNOWN inaccurate ascendant and Mars/Mercury/Venus longitudes. ` +
          `Treat this chart as degraded/low-confidence. Cause: ${String(err)}`,
      );
      const legacy = this.generateLegacy(dto, birthMoment.utc, coordinates, birthMoment);
      return this.stampDegraded(legacy, 'legacy-ascendant-and-fast-planet-accuracy-unverified');
    }
  }

  /**
   * Attach the standard degraded/accuracy markers to a legacy-path chart. Legacy calculations
   * are approximate and may be wrong near sign, nakṣatra, pāda, aspect, or house boundaries.
   * They must not be used for production-grade chart interpretation.
   */
  private stampDegraded(chart: GeneratedChart, reason?: string): GeneratedChart {
    const degradedReason =
      reason ?? 'legacy-ascendant-and-fast-planet-accuracy-unverified';
    const accuracy: AccuracyMetadata = {
      tier: 'approximate',
      degraded: true,
      degradedReason,
      notes: [
        'Legacy calculations are approximate and may be wrong near sign, nakṣatra, pāda, ' +
          'aspect, or house boundaries. They must not be used for production-grade chart interpretation.',
      ],
    };
    return {
      ...chart,
  ...
  private generateSwiss(
    ...
    const base = {
      ...
      chartData: {
        ...
        degraded: false,
        accuracyTier: 'ephemeris',
        accuracy: {
          tier: 'ephemeris',
          degraded: false,
          verifiedAgainst: ['Swiss Ephemeris (swisseph)'],
        } as AccuracyMetadata,
        inputLocalDateTime: birthMoment.inputLocalDateTime,
        calculationUtcDateTime: birthMoment.utc.toISOString(),
      },
    };
  ...
  private toUtcBirthMoment(
    ...
    const explicitTrimmed = explicitTimezone?.trim();
    if (explicitTrimmed && !this.isValidLuxonZone(explicitTrimmed)) {
      throw new ChartCalculationError(
        `Invalid or ambiguous IANA timezone string: "${explicitTrimmed}"`,
        'INVALID_TIMEZONE',
      );
    }
    ...
    if (process.env.CHART_TIMEZONE_SKIP === '1') {
      return {
        ...
        inputLocalDateTime: `${birthDate}T${birthTime}`,
      };
    }
    ...
    const zone = this.resolveBirthIanaZone(lat, lon, birthPlace, explicitTrimmed);
    ...
    if (!local.isValid) {
      console.warn(
        `[jyotisha-engine] Invalid birth datetime in ${zone}: ${local.invalidReason ?? 'unknown'}; using UTC wall clock`,
      );
      return {
        utc: this.toDateUtcWallClock(birthDate, birthTime),
        zoneUsed: 'UTC',
    ...
    return {
      ...
      inputLocalDateTime: local.toISO() ?? `${birthDate}T${birthTime}`,
    };
  }

  /** Latitude in [-90, 90], longitude in [-180, 180]; anything else cannot be a real birth location. */
  private validateCoordinates(lat: number, lon: number): void {
    if (!Number.isFinite(lat) || lat < -90 || lat > 90) {
      throw new ChartCalculationError(`Invalid latitude: ${lat} (must be between -90 and 90)`, 'INVALID_LATITUDE');
    }
    if (!Number.isFinite(lon) || lon < -180 || lon > 180) {
      throw new ChartCalculationError(`Invalid longitude: ${lon} (must be between -180 and 180)`, 'INVALID_LONGITUDE');
    }
  }
  ...
  private resolveBirthIanaZone(
    ...
    try {
      ...
      if (!sl) {
        console.warn(`[jyotisha-engine] geo-tz: no IANA zone for lat=${lat} lon=${lon}; using UTC`);
      }
    } catch (e) {
      console.warn(`[jyotisha-engine] geo-tz lookup failed (${String(e)})`);
    }
  ...
  /**
   * Vimśottarī Mahādaśā with classical birth-balance.
   *
   * The starting mahādaśā lord is the lord of the Moon's birth nakṣatra. Each nakṣatra spans
   * 13°20′ (360°/27); the *remaining* fraction of that span at birth determines how much of the
   * starting lord's full dasha years are left ("birth balance"): balance = fullYears × (remainingDeg / 13°20′).
   * Subsequent mahādaśās run their full classical duration in the fixed 9-lord Vimśottarī order
   * (total cycle = 120 years).
   */
  private computeMahadasha(birthDate: Date, moonLongitude: number): Record<string, unknown> {
    const nakSpan = 360 / 27;
    const nakIdx = Math.floor(moonLongitude / nakSpan);
    const nakLord = NAKSHATRA_LORDS[nakIdx % 9];
    const nakStart = nakIdx * nakSpan;
    const elapsedDegrees = moonLongitude - nakStart;
    const remainingDegrees = nakSpan - elapsedDegrees;
    const progress = elapsedDegrees / nakSpan;
    ...
    let segStartYearsFromBirth = 0;

    while (ageTracker > remaining) {
      segStartYearsFromBirth += remaining;
    ...
    const mahadashaStartDate = this.addYears(birthDate, segStartYearsFromBirth);
    const mahadashaEndDate = this.addYears(birthDate, segStartYearsFromBirth + remaining);

    // True notional start of the current mahādaśā lord's full classical span — for the first
    // (birth-balance) segment this predates birth by however much was already elapsed at birth.
    const isFirstSegment = segStartYearsFromBirth === 0 && current === nakLord;
    const notionalSpanStart = isFirstSegment
      ? this.addYears(birthDate, -(firstFullYears - firstRemaining))
      : mahadashaStartDate;

    const antaraWindow = this.computeAntaraWindow(current, yearsRemaining, notionalSpanStart);
    ...
    return {
      ...
      antara,
      birthNakshatra: NAKSHATRA_LIST[nakIdx % 27],
      birthNakshatraLord: nakLord,
      nakshatraElapsedDegrees: Number(elapsedDegrees.toFixed(4)),
      nakshatraRemainingDegrees: Number(remainingDegrees.toFixed(4)),
      firstDashaBalanceYears: Number(firstRemaining.toFixed(4)),
      mahadashaStartDate: mahadashaStartDate.toISOString(),
      mahadashaEndDate: mahadashaEndDate.toISOString(),
      ...(antaraWindow
        ? { antaraStartDate: antaraWindow.start.toISOString(), antaraEndDate: antaraWindow.end.toISOString() }
        : {}),
    };
  }
  ...
  /**
   * Calendar start/end of the *current* antardaśā, anchored to the notional (possibly pre-birth)
   * start of the enclosing mahādaśā's full classical span.
   */
  private computeAntaraWindow(
    dashaLord: (typeof NAKSHATRA_LORDS)[number],
    yearsRemainingInDasha: number,
    notionalSpanStart: Date,
    ...
    for (let i = 0; i < 9; i++) {
      const antaraLord = NAKSHATRA_LORDS[(startIdx + i) % 9];
      const antaraYears = (dashaYears * MAHADASHA_YEARS[antaraLord]) / 120;
      if (cursor + antaraYears > elapsedInDasha) {
        return {
          start: this.addYears(notionalSpanStart, cursor),
          end: this.addYears(notionalSpanStart, cursor + antaraYears),
        };
      }
      cursor += antaraYears;
    }
